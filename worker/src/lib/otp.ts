...
/**
 * A uniform 6-digit numeric PIN from the CSPRNG. Rejection sampling removes the small
 * modulo bias so every code from 000000–999999 is equally likely.
 */
...
/**
 * Keyed hash of a challenge secret, using the (random, per-flow) flow id as the HMAC key.
 * Only the hash is persisted, so a leak of DO storage never exposes the live PIN/token.
 */
